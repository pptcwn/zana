    ...
    <div className="fixed inset-0 bg-black/30 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white w-full sm:rounded-xl sm:max-w-xl max-h-[95vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between">
          <p className="text-sm font-semibold text-slate-900">สร้างออเดอร์ใหม่</p>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="space-y-3">
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">ลูกค้า</p>
          ...
          <div className="space-y-2">
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">สินค้า *</p>
            ...
            {items.map((item) => (
              <div key={item.product_id} className="flex items-center gap-3 py-2 border-b border-slate-50">
                ...
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => setItems((p) => p.map((i) => i.product_id === item.product_id ? { ...i, qty: Math.max(1, i.qty - 1) } : i))}
                    className="w-5 h-5 rounded border border-slate-200 text-slate-500 text-xs flex items-center justify-center hover:bg-slate-50">−</button>
                ...
                <button type="button" onClick={() => setItems((p) => p.filter((i) => i.product_id !== item.product_id))}
                  className="text-slate-300 hover:text-slate-500"><Trash2 size={13} /></button>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-slate-100">
            <div>
              ...
              <p className="text-xl font-semibold text-slate-900">{fmt(total)}</p>
            </div>
            <div className="flex gap-2">
              <button type="button" onClick={onClose} className="px-4 py-2 border border-slate-200 text-slate-500 rounded-md text-sm hover:bg-slate-50">ยกเลิก</button>
              <button type="submit" disabled={saving} className="px-4 py-2 bg-slate-900 text-white rounded-md text-sm font-medium hover:bg-slate-700 disabled:opacity-50 flex items-center gap-2">
...
function OrderPanel({ order, onClose }: { order: OrderRow; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end" onClick={onClose}>
      <div className="w-full max-w-sm bg-white h-full overflow-y-auto border-l border-slate-100" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="space-y-1">
            <p className="text-xs text-slate-400 uppercase tracking-wider font-medium">ลูกค้า</p>
            <p className="text-sm font-medium text-slate-900">{order.customers?.name}</p>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              <p className="text-xs text-slate-400 mb-0.5">ชำระ</p>
          ...
          {/* Items */}
          <div>
            <p className="text-xs text-slate-400 uppercase tracking-wider font-medium mb-2">สินค้า</p>
            <div className="space-y-1.5">
              {order.order_items?.map((item) => (
                <div key={item.id} className="flex justify-between text-sm">
                  <span className="text-slate-600">{item.product_name} ×{item.qty}</span>
                  <span className="text-slate-900">{fmt(item.unit_price * item.qty)}</span>
                </div>
              ))}
            </div>
          ...
          {/* Tracking */}
          <div>
            <p className="text-xs text-slate-400 uppercase tracking-wider font-medium mb-2">Tracking</p>
            <div className="flex gap-2">
              ...
              <button onClick={handleSaveTracking} disabled={saving || tracking === savedTracking}
                className="px-3 py-1.5 bg-slate-900 text-white rounded-md text-xs font-medium hover:bg-slate-700 disabled:opacity-40">
          ...
          {order.notes && (
            <div>
              <p className="text-xs text-slate-400 uppercase tracking-wider font-medium mb-1">หมายเหตุ</p>
          ...
          {order.status !== "cancelled" && order.status !== "completed" && (
            <div className="pt-3 border-t border-slate-100 space-y-2">
              <p className="text-xs text-slate-400 uppercase tracking-wider font-medium">เปลี่ยนสถานะ</p>
              <div className="flex gap-2">
                {order.status === "pending" && (
                  <button onClick={() => handleStatus("shipped")}
                    className="flex-1 border border-slate-200 text-slate-700 py-1.5 rounded-md text-xs hover:bg-slate-50 transition-colors">
                    ส่งแล้ว
                  </button>
                )}
                ...
                <button onClick={() => handleStatus("cancelled")}
                  className="flex-1 border border-slate-200 text-slate-400 py-1.5 rounded-md text-xs hover:bg-slate-50 transition-colors">
                  ยกเลิก
                </button>
              </div>
            </div>
  ...
  return (
    <div className="space-y-5 max-w-5xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-base font-semibold text-slate-900">Orders</h1>
          <p className="text-xs text-slate-400 mt-0.5">จัดการออเดอร์ทั้งหมด</p>
        </div>
        <button onClick={() => setShowCreate(true)}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-900 text-white rounded-md text-sm font-medium hover:bg-slate-700 transition-colors">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={label} className="bg-white border border-slate-100 rounded-lg px-4 py-3 flex items-center gap-3">
            <span className={`w-2 h-2 rounded-full shrink-0 ${dot}`} />
            <span className="text-sm text-slate-500">{label}</span>
            <span className="ml-auto text-lg font-semibold text-slate-900">{count}</span>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <div className="relative flex-1 min-w-48">
          ...
          <input type="text" placeholder="ค้นหา..." value={search} onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-8 pr-3 py-1.5 border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-slate-900 text-slate-800 placeholder:text-slate-300 bg-white" />
        </div>

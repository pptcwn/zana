...
function ProfitCalculator({ adSpendToday, todayRevenue }: { adSpendToday: number; todayRevenue: number }) {
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-lg p-5">
      <p className="text-sm font-medium text-slate-900 mb-4">Profit Calculator</p>
        ...
        {[
          ...
          <div key={label}>
            ...
            <input
              ...
              className="w-full border border-slate-200 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-slate-900"
            />
          </div>
        ))}
      </div>
      <div className="grid grid-cols-4 gap-2 pt-3 border-t border-slate-100">
        {[
          ...
          <div key={label}>
            ...
            <p className={`text-sm font-semibold mt-0.5 ${muted ? "text-red-500" : "text-slate-900"}`}>{value}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div>
        ...
        <p className="text-xs text-slate-400 mt-0.5">ภาพรวมธุรกิจ</p>
      </div>
      ...
      {/* Chart */}
      <div className="bg-white border border-slate-100 rounded-lg p-5">
        <p className="text-sm font-medium text-slate-900 mb-4">Revenue & Profit เดือนนี้</p>
        {data.dailySales.length === 0 ? (
          <div className="h-48 flex items-center justify-center text-xs text-slate-300">ยังไม่มีข้อมูล</div>
        ) : (
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={data.dailySales} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
              <defs>
                {[{ id: "r", color: "#0f172a" }, { id: "p", color: "#64748b" }].map(({ id, color }) => (
              ...
              <YAxis tickFormatter={(v) => `฿${(v / 1000).toFixed(0)}K`} tick={{ fontSize: 11, fill: "#94a3b8" }} axisLine={false} tickLine={false} width={45} />
              <Tooltip content={<CustomTooltip />} />
              <Area type="monotone" dataKey="revenue" name="Revenue" stroke="#0f172a" strokeWidth={1.5} fill="url(#r)" dot={false} />
              <Area type="monotone" dataKey="profit" name="Profit" stroke="#64748b" strokeWidth={1.5} fill="url(#p)" dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </div>
      ...
      {data.platformSales.length > 0 && (
        <div className="bg-white border border-slate-100 rounded-lg p-5">
          <p className="text-sm font-medium text-slate-900 mb-4">Sales by Platform เดือนนี้</p>
            ...
            <BarChart data={data.platformSales} layout="vertical" margin={{ left: 0, right: 0 }}>
              ...
              <Bar dataKey="revenue" name="Revenue" radius={[0, 3, 3, 0]}>
                {data.platformSales.map((_, i) => (
                  <Cell key={i} fill={i === 0 ? "#0f172a" : i === 1 ? "#334155" : i === 2 ? "#64748b" : "#94a3b8"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
